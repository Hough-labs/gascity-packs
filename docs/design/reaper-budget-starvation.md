...
Status: **plan only, not dispatchable yet.** `gcp-jxtc` is in flight on
`polecat-worktree-reap.sh` (polecat `gastown__polecat-gc-yhsv`, branch
`polecat/gcp-jxtc`). The patch described here is cut *against jxtc's result*,
not against today's HEAD. Nothing below authorises an edit to that script now.
...
`polecat-worktree-reap.sh:673-706` rotates the sorted candidate list to start
after the last-decided candidate, and `:1533-1546` persists it. A full pass
...
The cursor advances **only when a candidate is decided**. From `:1540`:
...
1. The bulk bead read at `:759` consumes what is left of the 8s budget.
...
| Site | Call |
|---|---|
| `:759` | bulk `gc bd show <ids> --json` — the gate read |
| `:~890` | `gc session list` — the roster/liveness read |
| `:969` | bulk `gc bd show <ids> --json` — the pre-removal recheck read |
...
- `GC_BD=(gc bd --rig "$RIG_NAME")` at `:392` becomes a bare `bd` invocation
...
1. **Deterministic budget tests, not timing tests.** Drive the script with
   `--budget` (a plain flag, `:243`) and `GC_REAP_BUDGET_SECONDS` (`:136`) at
